...
import time
import numpy as np
from util.util import quaternion_multiply, quaternion_invert, matrix_to_quaternion, exp_decay_alt, quaternion_similarity, quaternion_to_matrix
...
class WeldingEnvironmentPybullet(gym.Env):
    ...
    def reset(self):
        ...
        #### temp
        visual_id = pyb.createVisualShape(shapeType=pyb.GEOM_SPHERE, radius=0.05, rgbaColor=[1,0,0,1])
        point_id = pyb.createMultiBody(
                    baseMass=0,
                    baseVisualShapeIndex=visual_id,
                    basePosition=np.array([2,  1.87400000e+00,  8.26572114e-01]),
                    )
    ...
    def _set_plan(self):
        ...
        if self.goals:
            ...
            for idx in range(len(goal["weldseams"])):
                ...
                tpl = (np.array([2,  1.87400000e+00,  8.26572114e-01]), goal["norm"][idx], goal["target_rot"][idx], goal["tool"], target_pos_bas)
    ...
    def _init_settings(self):
        ...
        self.ee_pos_reward_thresh = 5e-1  # might need adjustment
    ...
    def _init_gym_vars(self):
        ...
        min_action = np.array([-1, -1, -1, -1, -1, -1 ]).astype(np.float32) #tbd
...
class WeldingEnvironmentPybulletLidar2(WeldingEnvironmentPybulletLidar):
    ...
    def __init__(self,
        ...
        low = np.hstack([min_joints, min_position, np.ones(4) * -1, min_objective_position]).astype(np.float64)
        high = np.hstack([max_joints, max_position, np.ones(4), max_objective_position]).astype(np.float64)

        #self.observation_space = gym.spaces.Box(low=low, high=high, shape=(34,), dtype=np.float32)

        obs_spaces = {
            'position': gym.spaces.Box(low=-2, high=2, shape=(14,), dtype=np.float32),
    ...
    def _get_obs(self, normalize=False):
        ...
        lidar_results = self._get_lidar_probe()
        if self.objective:
            state = np.hstack((self.get_joint_state(), np.array(tmp[0]) - np.array([2,  1.87400000e+00,  8.26572114e-01]), self._quat_ee_to_w(np.array(tmp[1])), np.linalg.norm(np.array(tmp[0]) - np.array([2,  1.87400000e+00,  8.26572114e-01])))).astype(np.float32)
        else:
            state = np.hstack((np.array(tmp2[4][:2]), np.array(tmp[0]), pyb.getEulerFromQuaternion(self._quat_ee_to_w(np.array(tmp[1]))), self.get_joint_state(), [0, 0, 0])).astype(np.float32)
    ...
    def __perform_action(self, action):
        ...
        if action is not None:
            ...
            # unfortunately, the order of dict entries matters to the gym contains() method here
            # if somehow the order of dict entries in the observation space OrderedDict changes, then the order of the next lines defining the entries of the new state needs to be switched as well
            new_ee_position = np.array(tmp[0]) + action[:3]
            current_rotation_as_rpy = pyb.getEulerFromQuaternion(state["position"][9:13])
            new_rotation_as_rpy = np.array([entry + action[3:][idx] for idx, entry in enumerate(current_rotation_as_rpy)])
            new_rotation_as_quaternion = pyb.getQuaternionFromEuler(new_rotation_as_rpy)
            new_rotation_as_quaternion_in_correct_frame = self._quat_w_to_ee(new_rotation_as_quaternion)         
            # TODO: reimplement the valid bounds check once the bounds have actually been settled on sometime in the future
    ...
    def _perform_action(self, action):
        if action is not None:
            ...
            # move the joints
            if True:
                timeout = self.movej(self.get_joint_state() + new_joints*0.2)
            else:
                timeout = self.movej(new_joints)
            if timeout:
                return timeout
    ...
    def reward(self, obs):
        ...
        if self.path_state == 2:
            ...
            objective_with_slight_offset = self.objective[0] + self.objective[1][0] * 0.01 + self.objective[1][1] * 0.01
            objective_with_slight_offset = np.array([1.96945097e+00,  1.87400000e+00,  8.26572114e-01])
            distance = np.linalg.norm(obs["position"][6:9]) 
        if distance < self.ee_pos_reward_thresh:
            reward = 1
            pos_done = True  # objective achieved
        else:
